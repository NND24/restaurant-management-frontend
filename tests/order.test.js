...
import { createPendingOrder, getOrderById, findItemToEdit } from "./utils/apiClient.js";
import { login, expectVerifyTabHasOrder, expectHistoryTabHasOrder } from "./utils/helpers.js";

const CLIENT_EMAIL = process.env.STAFF_EMAIL || "n21dccn003@student.ptithcm.edu.vn";
...
test("TC07 _ Kiểm tra tab 'mới nhất' có đơn hàng vừa đặt không", async ({ page }) => {
  ...
  if (!apiOrderData || !Array.isArray(apiOrderData.items)) {
    throw new Error(`Expected items array in API response, got: ${JSON.stringify(orderApi)}`);
  }

  // totals
  const expectedQty = apiOrderData.items.reduce((a, i) => a + (i.quantity || 0), 0);
  const expectedPrice = apiOrderData.items.reduce((acc, item) => {
    const base = (item.dish?.price || item.price || 0) * item.quantity;
    const toppings =
      (Array.isArray(item.toppings) ? item.toppings.reduce((s, t) => s + (t.price || 0), 0) : 0) * item.quantity;
    return acc + base + toppings;
  }, 0);

  await expect(row).toHaveAttribute("data-total-qty", String(expectedQty));
  await expect(row).toHaveAttribute("data-total-price", String(expectedPrice));
  ...
  for (let i = 0; i < uiCount; i++) {
    ...
    const uiTop = Number(await uiItem.getAttribute("data-item-topping-count"));
  ...
  if (orderApi.user?.name) {
    await expect(row.getByTestId("customer-name")).toHaveText(orderApi.user.name, { useInnerText: true });
  }
});
...
test("TC8 _ Kiểm tra tab 'đã xác nhận' có đơn hàng với trạng thái đã xác nhận", async ({ page }) => {
  ...
  const pendingRow = page.locator(`[data-testid="order-row"][data-order-id="${orderId}"]`);
...
test("TC11 _ Kiểm tra chỉnh sửa order", async ({ page }) => {
  ...
  const originalToppingNames = new Set((originalItem.toppings ?? []).map((t) => t.toppingName));
  console.log(`Original item: ${dishName} x ${originalQty}, toppings: ${[...originalToppingNames].join(", ")}`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    const alreadyHad = [...originalSet].some((t) => norm.includes(t) || t.includes(norm));
    if (!alreadyHad) {
  ...
  await expect(page.locator(`text=/^\\s*${expectedQty} x ${dishName}\\s*$/`)).toBeVisible();

  // Find the LI that contains the quantity + dish name
  const dishRow = page.locator("li", { hasText: `${expectedQty} x ${dishName}` }).first();
  ...
  await expect(dishRow.locator("span", { hasText: toppingNameForAssert })).toBeVisible();
  ...
  const updatedItem =
    (updatedOrder.items || []).find((it) => (it.dish?.name || it.dishName) === dishName) || updatedOrder.items?.[0];

  expect(updatedItem?.quantity, "Quantity should increase by 1").toBe(expectedQty);

  const updatedToppingNames = (updatedItem?.toppings ?? []).map((t) => t.toppingName);

  // VN accents, spacing
  const serverHasPicked = updatedToppingNames
    .map((n) => normalize(n))
    .some((n) => n === normalize(toppingNameForAssert) || normalize(toppingNameForAssert).includes(n));

  expect(serverHasPicked, `New topping should be present. Saw: ${updatedToppingNames.join(", ")}`).toBe(true);
});
