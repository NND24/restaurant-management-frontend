...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [dishRes, toppingRes, allToppingsRes, categoriesRes] = await Promise.all([
          getDish(id),
  ...
  const handleToppingToggle = (toppingId) => {
    setSelectedToppings((prev) => {
      ...
      updatedSet.has(toppingId) ? updatedSet.delete(toppingId) : updatedSet.add(toppingId);
      return updatedSet;
    });
  };
        ...
        <div className='flex-1 overflow-auto space-y-6'>
            ...
            >
              <div className='bg-white p-5 rounded-lg shadow-lg p-10' onClick={(e) => e.stopPropagation()}>
                ...
                <div className='flex justify-end gap-3 mt-4'>
                  ...
                  <button onClick={confirmSave} className='bg-green-500 text-white px-4 py-2 rounded'>
                    Xác nhận
                  </button>
                </div>
              </div>
          ...
          {showDeleteModal && (
            ...
            >
              <div className='bg-white p-5 rounded-lg shadow-lg p-10' onClick={(e) => e.stopPropagation()}>
                <p>
                  Bạn có chắc chắn muốn <strong>xóa</strong> món ăn này?
                </p>
                <div className='flex justify-end gap-3 mt-4'>
                  <button
                    onClick={() => setShowDeleteModal(false)}
