...
const StoreTime = ({ openHour, closeHour, openStatus, onUpdateTime, onToggle }) => {
  ...
  return (
    <div className='mx-auto max-w-full space-y-6 rounded-xl bg-white p-6 shadow-md'>
      <div className='flex items-center justify-between'>
        <h2 className='text-xl font-semibold text-gray-800'>Giờ hoạt động cửa hàng</h2>
        {!isEditing && (
          <button
      ...
      <label className='inline-flex cursor-pointer items-center'>
        <input type='checkbox' className='peer sr-only' checked={openStatus === "OPEN"} onChange={onToggle} />
        ...
        <span className='ms-3 text-sm font-medium text-gray-900'>Hoạt động</span>
      </label>
      ...
      <div className='flex items-center justify-between'>
        ...
        {isEditing ? (
          <input
            type='time'
            id='openHour'
            className='w-2/3 rounded-md border px-3 py-2 focus:ring-2 focus:ring-orange-500'
            value={tempOpen}
